...
exports.getAdvisors = async (req, res) => {
    try {
        const [advisors] = await promisePool.query(
...
exports.createAdvisor = async (req, res) => {
    try {
        ...
        if (roles.length === 0) {
            return res.status(500).send({ message: "Role 'advisor' introuvable en base de données !" });
        }
        ...
        res.status(201).send({ message: 'Compte conseiller créé avec succès !', id: userId });
    } catch (error) {
        res.status(500).send({ message: error.message });
    }
};
...
exports.deleteAdvisor = async (req, res) => {
    try {
        ...
        res.status(200).send({ message: 'Compte conseiller supprimé avec succès !' });
    } catch (error) {
        res.status(500).send({ message: error.message });
    }
};
